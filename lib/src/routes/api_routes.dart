...
class ApiRoutes {
  ...
  final IoTController _iotController =
      IoTController(); // ✅ CORRIGIDO: classe correta
  ...
  void _setupRoutes() {
    ...
    _router.get(
        '/api/iot/locations-all', _iotController.getAllCurrentLocations);
    _router.get('/api/iot/performance-test/<employeeId>',
        (Request request, String employeeId) async {
  ...
  Future<Response> _getSystemStats(Request request) async {
    try {
      _logger
          .info('📊 GET /api/stats - Estatísticas do sistema (IoT V2 Final)');
      ...
      final stats = {
        ...
        'endpoints': {
          ...
          'iot_location_employee':
              '/api/iot/location/:employeeId (current only)',
