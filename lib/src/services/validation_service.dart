...
class ValidationResult {
  ...
  ValidationResult.success() : isValid = true, error = null, details = null;
...
class ValidationService {
  ...
  static ValidationResult validateEmployeeId(String? id) {
    ...
    if (!RegExp(r'^[A-Z0-9]+$').hasMatch(id.trim())) {
      return ValidationResult.error('ID deve conter apenas letras maiúsculas e números');
    }
  ...
  static ValidationResult validateDataAdmissao(DateTime? data) {
    ...
    if (data.isBefore(dataMinima)) {
      return ValidationResult.error('Data de admissão muito antiga (mais de 50 anos)');
    }
  ...
  static ValidationResult validateEmployeeUpdate(Map<String, dynamic> json) {
    ...
    if (errors.isNotEmpty) {
      return ValidationResult.error('Dados de atualização inválidos', errors.join('; '));
    }
