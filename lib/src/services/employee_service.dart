...
class EmployeeService {
  ...
  Future<List<Employee>> getAllEmployees() async {
    try {
      ...
      if (employees.isNotEmpty) {
        ...
        _logger.info('📊 Status: $ativos ativos, $veteranos veteranos de ${employees.length} total');
      }
  ...
  Future<Employee?> getEmployeeById(String id) async {
    try {
      ...
      if (!idValidation.isValid) {
        throw InvalidEmployeeDataException('ID inválido: ${idValidation.error}');
      }
  ...
  Future<Employee> createEmployee(Map<String, dynamic> employeeData) async {
    try {
      ...
      final validation = ValidationService.validateEmployeeCreation(employeeData);
      if (!validation.isValid) {
        throw InvalidEmployeeDataException(validation.error!, details: validation.details);
      }
      ...
      final existingByEmail = await _repository.getEmployeeByEmail(employee.email);
  ...
  Future<Employee> updateEmployee(String id, Map<String, dynamic> updateData) async {
    try {
      ...
      if (updateData.containsKey('email')) {
        ...
        if (newEmail != existing.email) {
          final existingByEmail = await _repository.getEmployeeByEmail(newEmail);
  ...
  Future<Map<String, dynamic>> getStatistics() async {
    try {
      ...
      if (totalFuncionarios != null && funcionariosAtivos != null && totalFuncionarios > 0) {
        result['taxa_ativacao'] = ((funcionariosAtivos / totalFuncionarios) * 100).round();
      } else {
  ...
  void _validateBusinessRules(Employee employee) {
    ...
    if (employee.tempoEmpresaAnos > 10) {
      _logger.info('⭐ Funcionário especial detectado: ${employee.nome} (${employee.tempoEmpresaAnos} anos)');
    }
...
class InvalidEmployeeDataException implements Exception {
  ...
  @override
  String toString() => 'InvalidEmployeeDataException: $message${details != null ? ' ($details)' : ''}';
}
